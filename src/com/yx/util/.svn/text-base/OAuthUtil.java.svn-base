...
public class OAuthUtil extends WeixinUtil{
	...
	private static String appId="wx4fbdc6894080873b";
	private static String appSecret="81b4cb36f2220fd4ed895b39ffe9fc55";
	/**
	...
	public static JSONObject getSNSUserInfo(String accessToken,String openId,String fromUserName) {
		SNSUserInfo sui=null;
		String requestUrl=OAUTH2_USERINFO_URL.replace("ACCESS_TOKEN", accessToken).replace("OPENID", openId);
		// 通过网页授权获取用户信息
		JSONObject jsonObject=httpRequest(requestUrl, "GETa", null);
//		User user=null;
		if (null!=jsonObject) {
			try {
//				user=new User();
				...
				sui=new SNSUserInfo();
				// 用户标识
				sui.setOpenId(jsonObject.toString());
				// 昵称
				...
				//用户微信号
				//sui.setWechat(fromUserName);
//				user.setSex(sui.getSex());
//				user.setNickname(sui.getNickname());
//				user=dao.findByWechat(fromUserName);
//				dao.update(user);
			} catch (Exception e) {
				sui=null;
	...
	public static void main(String[] args) {
		WeixinOauth2Token weixinOauth2Token=getOauth2AccessToken( "0011d88ec32c1ab93fcd8dfa2e138b4e");
...
//		User snsUserInfo=getSNSUserInfo("111111", "111111","1");
